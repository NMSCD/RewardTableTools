...
function rewardChances(EXMLSection, outputId, inputType, xmlDoc) {
	...
	for (const entry of entries) {
		...
		} else if (entry.querySelector('[name="Stat"]')) {
			IDs.push(`${entry.querySelectorAll('[name="ModifyType"]')[1].getAttribute("value")} stat: ${entry.querySelector('[name="Stat"]').getAttribute("value")} (${entry.querySelector('[name="Amount"]').getAttribute("value")})`);
