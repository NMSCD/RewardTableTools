...
function buildTable(data) {
	...
	for (let i = 0; i < data.IDs.length; i++) {
		...
		const outString = `<div>${i + 1}.</div><div>${rewardType}</div><div>${itemId}</div><div>${chance}%</div>`;
...
function getRewards(EXMLSection) {
	...
	const selectors = ['ID', 'ProductList', 'Items', 'ProductIds', 'TechList', 'Currency', 'ProceduralProductCategory', 'Group', 'TechId', 'Event', 'Stat', 'CreatureID', 'Reward'];
